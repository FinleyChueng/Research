import util.netutils as netutil
...
class DqnAgent(Model):
    ...
    def __init__(self,
                 ...
                 name_scope='DrqnAgent'):
        r'''
            The initialization method of implementation. It just expand two additional parameters.

        Parameters:
            input_dim: The dimension of input layer of the model.
            output_dim: The dimension of output layer of the model.
            image_width: The width of input image.
            image_height: The height of input image.
            image_channels: The channel number of input image.
            heads_num: The head number of "Ensemble-CNN".
            action_history_len: The length of action history, which is as a part of input.
            name_scope: The global name scope of the model.
        '''

        Model.__init__(self, name_scope)

        # Normal Initialization.
        self._input_size = input_image_size
        self._act_dim = action_dim
        self._cls_dim = clazz_dim
        self._feature_stride = None

        # Specify the dimension (number) of class-based mask.
        if use_all_masks:
            self._mask_dim = clazz_dim
        else:
            self._mask_dim = clazz_dim - 1

        # Specify the feature extraction network.
        if not isinstance(feature_extraction_network, FeatureExtractionNetwork):
            raise TypeError('The feature_extraction_network must inherited from'
                            'FeatureExtractionNetwork !!!')
        self._fe_net = feature_extraction_network

        # The name scope specified to support the "Step-wise Training".
        self._base_name_scope = None

        # The name scope pair specified to support the "Double DQN".
        self._DDQN_namescpoe_pair = None

        # The proportion value, which is used to control the proportion of
        #   low frequency signal for "OCT" fusion method.
        self._oct_alpha = 0.125

        # The hidden layer size for the GRU cell.
        self._gru_hsize = 1024

        # Define the value (Scalar) for batch size and the track length of input frame (bbox region).
        #   Coz we use the LSTM, we have to determine them at the very beginning.
        # -------------------------------------------------------------------------------------------
        # Note that, we have the two values respects to the "Training" and "Inference" phrase.
        self._train_batch_size = train_batch_size
        self._train_track_len = train_track_len
        # Infer.
        self._infer_batch_size = infer_batch_size
        self._infer_track_len = infer_track_len
        ...
        self._feature_stride = None
        self._image = None
        self._fe_output = None
        # The DRQN holders.
        self._drqn_in = None
        self._drqn_gru_Sin = None
        self._drqn_gru_state = None
        self._drqn_output = None
        # UN holders.
        self._selective_mask = None
        self._un_output = None
        self._un_score_maps = None

        # -------------- The holders for loss-related operations. --------------
        # The DRQN related holders.
        self._target_image = None
        self._target_drqn_in = None
        self._target_drqn_gru_Sin = None
        self._target_drqn_output = None

        # Regularization Related.
        if enable_regularization:
            self._l2_regularizer = tf.contrib.layers.l2_regularizer(1.0)    # Add real coef in compute loss.
        else:
            self._l2_regularizer = None
    ...
    def action_dim(self):
        r'''
            Get the action dimension of the DRQN.
        '''
    ...
    def mask_dim(self):
        r'''
            Get the dimension of the input class-based mask.
        '''
    ...
    def gru_hsize(self):
        r'''
            Get the hidden layer size of the GRU cell.
        '''
        return self._gru_hsize
    ...
    def definition(self,
        ...
        # Firstly get the step-wise train name scope.
        if stepwise_training_name_scope is None:
            # Default to be empty str.
            self._base_name_scope = ''
        else:
            # Check the type.
            if not isinstance(stepwise_training_name_scope, str):
                raise TypeError('The stepwise_training_name_scope must be a str or NoneType !!!')
            # Assign the value.
            self._base_name_scope = stepwise_training_name_scope + '/'
        ...
        self._ios_dict = self.__architecture(fuse_RF_feats=fuse_RF_feats,
                                             dqn_name_scope_pair=dqn_name_scope_pair,
    ...
    def notify_copy2_DDQN(self, tf_sess, only_head):
        ...
        #     ops_base = netutil.copy_model_parameters(from_namespace, to_namespace)
    ...
    def __architecture(self, fuse_RF_feats, dqn_name_scope_pair, dueling_network):
        r'''
            Construct the architecture. Specify by the parameters.
    ...
    def __build_Feature_Extract_Net(self, name_scope, fuse_RF_feats):
        r'''
            Build the "Feature Extract" network. Which outputs the deep feature maps of
                raw input image.
        ...
        # Define the placeholder of the raw image.
        image = tf.placeholder(tf.float32, [None, self._input_size[0], self._input_size[1], 4],
                               name=name_scope + '_Image')  # 240, 240, 4

        # Definition
        with tf.variable_scope(name_scope):

            # Assign the proportion of output channels according to the
            #   "RF Feature Fusion" flag.
            if fuse_RF_feats:
                feats_pro = self._oct_alpha
            else:
                feats_pro = 1.
            ...
            fe_output, \
                feature_stride,\
                feats_dict = self._fe_net.build(image,
                                                self._l2_regularizer,
                                                name_scope,
                                                arg=(feats_pro, self._train_phrase))    # 15,15,256

        print('### Finish the definition of FEN (name scope: {}). The output shape: {}'.format(
            name_scope, fe_output.shape))

        # Finish the definition of FEN. Meanwhile return the
        #   Input holders: Raw image.
        #   Output holders: FEN output, feature maps stride, feature dictionary.
        return image, fe_output, feature_stride, feats_dict
    ...
    def __build_Upsample_Net(self, name_scope, feats_dict):
        r'''
            Build the "Up-sample" network, which is actually the real "Segmentation"
                network. Note that, we will use the features from "Feature Extract"
                network to refine the segmentation result. What's more, this network
                will take the mask produced by "Deep Recurrent Q" network to filter
                the features. To reach the better segmentation result.
        ...
        # Check the validity.
        if self._fe_output is None:
            raise Exception('One must build the FEN first !!!')

        # Calculate the input size of the DRQN. It's actually the size (shape)
        #   of the output feature maps of FEN.
        UN_inw = self._input_size[0] // self._feature_stride
        UN_inh = self._input_size[1] // self._feature_stride
        UN_inc = self._fe_output.shape.as_list()[-1]
        # Check validity.
        assert UN_inw == self._fe_output.shape.as_list()[1]
        assert UN_inh == self._fe_output.shape.as_list()[2]

        # Define the placeholder for "Selective Mask" produced by "Deep Recurrent Q Network",
        #   which is actually the result of DRQN. We use it to filter the feature maps of
        #   "Feature Extract Network".
        selective_mask = tf.placeholder(tf.float32, [None, self._input_size[0], self._input_size[1], self._mask_dim],
                                        name=name_scope + '_Selective_mask')    # [bat_size * track_len, inw, inh, mask]
    ...
    #### --------------------- LSTM Related ----------------------------
    def __get_lstm_cell(self, lstm_size, keep_prob):
        lstm = tf.nn.rnn_cell.BasicLSTMCell(num_units=lstm_size)
        return tf.nn.rnn_cell.DropoutWrapper(cell=lstm, output_keep_prob=keep_prob)

    #### --------------------- CNN Related -----------------------------
    def __weight_variable(self, shape, name, enable_regularization=False):
        r'''
            Generate the weight variable with the specific shape vector.

        ------------------------------------------------------------------------
        Parameters:
            shape: The specific shape indicating the dimension of the weight
                variable. The shape is consisted of (1-dim, 2-dim, ...).

        -------------------------------------------------------------------------
        Return:
            The weight variable of specific shape.
        '''

        initial = tf.truncated_normal(shape, stddev=0.01, name=name+'_var_init')
        if enable_regularization:
            weights = tf.get_variable(name+'_var', initializer=initial, regularizer=self._l2_regularizer)
        else:
            weights = tf.get_variable(name+'_var', initializer=initial)
        return weights

    def __bias_variable(self, shape, name):
        r'''
            Generate the bias variable with the specific shape vector.

        ------------------------------------------------------------------------
        Parameters:
            shape: The specific shape indicating the dimension of the bias
                variable. The shape is consisted of (1-dim, 2-dim, ...).

        -------------------------------------------------------------------------
        Return:
            The bias variable of specific shape.
        '''

        initial = tf.constant(0.01, shape=shape, name=name+'_bias_init')
        return tf.get_variable(name=name+'_bias', initializer=initial)

    def __fully_connected_layer(self, input_tensor, output_channels, var_name, name_scope, use_relu=False):
        r'''
        ...
        Parameters:
            ...
            name_scope: The name scope.
            use_relu: Whether should pass through a "Relu" layer.
        ...
        W = self.__weight_variable([int(input_tensor.shape[-1]), output_channels], name=var_name)
        b = self.__bias_variable([output_channels], name=var_name)
        out_tensor = tf.add(tf.matmul(input_tensor, W), b,
                            name=name_scope + '_' + var_name)  # [None, output_channels]
        if use_relu:
            out_tensor = tf.nn.relu(out_tensor, name=name_scope + '_' + var_name + '_relu')
        return out_tensor
